...
const addWord = (req, response) =>{
 ...
 var use_yn = "T"
 var word_icon = req.body.word_icon
 var category_id = req.body.category_id
   pool.query('INSERT INTO words(word_name, use_yn, word_icon, category_id) VALUES ($1, $2, $3, $4)',[word_name,use_yn,word_icon,category_id],(error,results)=>{
     if(error){
       console.log(error);
       response.json({
          status: 410,
...
const wordById = (request, response) =>{
    ...
    pool.query('Select * from words where word_id = $1',[id], (error, result)=>{
      if(error){
        throw error
      }
